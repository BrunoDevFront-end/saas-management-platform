...
const CANVAS = { width: 140, height: 220 };
const LAYOUT = {
  head: { x: 23, y: -6, w: 94, h: 74 },
  body: { x: 22, y: 64, w: 96, h: 137 },
  armLeft: { x: 5, y: 80, w: 18, h: 104 }, // braço E
  armRight: { x: 117, y: 80, w: 17, h: 105 }, // braço D
  // centro do visor preto, em coordenadas locais do <svg> recortado da cabeça (303..397, -1..73)
...
export default function AnyMascot({
  ...
  // 2. Piscar — loop de blink, pausa durante o hover pra não brigar com a reação
  useEffect(() => {
    let cancelled = false;
    const blinkLoop = async () => {
      while (!cancelled) {
        ...
        if (!cancelled) setEyeState("default");
      }
    };
    blinkLoop();
    return () => {
  ...
  const handleHoverStart = useCallback(() => {
    ...
    armControls.start("wave");
  }, [interactive, armControls]);

  const handleHoverEnd = useCallback(() => {
    setIsHovering(false);
    setEyeState("default");
    armControls.start("rest");
  }, [armControls]);
      ...
      >
        ...
        <g transform={`translate(${LAYOUT.body.x}, ${LAYOUT.body.y})`}>
          ...
          >
            ...
            />
            <path
              d="M3.17973 6.6841C15.6797 2.01743 50.9798 -4.5159 92.1798 6.6841C99.3798 23.8841 90.1798 65.8508 84.6798 84.6841C81.6798 94.6841 70.1798 122.184 59.1798 131.184C49.5379 139.073 39.6798 135.017 35.1798 131.184C1.57978 85.9841 -3.82026 24.1841 3.17973 6.6841ZM92.1798 6.6841C87.3798 16.2841 62.1798 19.1841 46.6798 19.1841C29.8798 19.9841 9.17973 14.1841 3.17973 6.6841"
              stroke="black"
            />
          </svg>
        </g>

        {/* Cabeça (recortada) + olhos */}
        <g transform={`translate(${LAYOUT.head.x}, ${LAYOUT.head.y})`}>
          ...
          >
            ...
            />
            <path
              d="M0.5 107.501V110.501M350.5 72.0014C293.5 65.5014 306.667 49.5014 304.5 42.5014C304.5 30.3347 313.7 4.9014 350.5 0.501404C364.667 1.16807 393.6 10.5014 396 42.5014C396 58.5014 396 66.0014 350.5 72.0014Z"
              stroke="black"
            />
            <path
          ...
          {/* olhos, centralizados no visor preto da cabeça */}
          <g
            transform={`translate(${LAYOUT.eyesCenter.x}, ${LAYOUT.eyesCenter.y})`}
          >
            <EyePair state={eyeState} />
          </g>
        </g>

        {/* braço E (esquerdo) — desenhado por cima do corpo/cabeça de propósito */}
        <g transform={`translate(${LAYOUT.armLeft.x}, ${LAYOUT.armLeft.y})`}>
          <motion.g
            variants={{ rest: { rotate: 0 }, wave: { rotate: [-4, -14, -4] } }}
            animate={armControls}
            initial="rest"
            transition={{
              duration: 0.7,
...
// cada variante é centralizada em (0,0) a partir do próprio width/height, pra alinhar certinho no visor
function EyePair({ state }: { state: AnyEyeState }) {
  switch (state) {
    case "happy-circle": // olho circular feliz.svg (48x13)
      return (
    ...
    case "happy-v": // olho feliz em v.svg (51x15)
      return (
    ...
    case "sleepy": // olho dormindo.svg (47x3)
      return (
    ...
    default: // olho padrão.svg (49x14)
      return (
        <g transform="translate(-24.5, -7)">
          <ellipse cx="9.5" cy="7" rx="9.5" ry="7" fill="#489BE9" />
          <ellipse cx="39.5" cy="7" rx="9.5" ry="7" fill="#489BE9" />
        </g>
      );
  }
}
